...
import { Trash2, Star, Filter, Search, MessageSquare } from 'lucide-react';
import DashboardLayout from '../../layouts/DashboardLayout';
import Card from '../../components/Card';
import { useFeedback } from '../../contexts/FeedbackContext';

const AdminFeedback = () => {
  const { feedbackList, deleteFeedback } = useFeedback();
  const [search, setSearch] = useState('');
  const [typeFilter, setTypeFilter] = useState('all');

  const filteredFeedback = feedbackList.filter(fb => {
    const matchesSearch = fb.comment.toLowerCase().includes(search.toLowerCase()) ||
                ...
                <div className="flex flex-col md:flex-row justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <p className="font-bold text-white">{fb.userName}</p>
                        <p className="text-xs text-zinc-500">{fb.userEmail}</p>
                      </div>
                    </div>
